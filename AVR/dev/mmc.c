...
{
	b32 baddr;
	baddr.b32_1 = addr << 9; //addr = addr * 512 
...
{   
	...
	vinf->sector_count = sectorBuffer[6] & 0x03;
	vinf->sector_count <<= 8;
	vinf->sector_count += sectorBuffer[7];
	vinf->sector_count <<= 2;
	vinf->sector_count += (sectorBuffer[8] & 0xc0) >> 6;

	// get the val for C_SIZE_MULT. bits [49:47] of sectorBuffer
	// [49:48] == sectorBuffer[5] && 0x03
	// [47]    == sectorBuffer[4] && 0x80
	vinf->sector_multiply = sectorBuffer[9] & 0x03;
	vinf->sector_multiply <<= 1;
	vinf->sector_multiply += (sectorBuffer[10] & 0x80) >> 7;
	...
	vinf->name[0] = sectorBuffer[3];
	vinf->name[1] = sectorBuffer[4];
	vinf->name[2] = sectorBuffer[5];
	vinf->name[3] = sectorBuffer[6];
	vinf->name[4] = sectorBuffer[7];
...
{  
	...
	b32 baddr;
	baddr.b32_1 = addr << 9; //addr = addr * 512 
	CMD[1] = baddr.b8_4.b8_4;
	CMD[2] = baddr.b8_4.b8_3;
	CMD[3] = baddr.b8_4.b8_2;
#endif

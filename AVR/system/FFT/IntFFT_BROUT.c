...
// Function PROTOTYPES
//-----------------------------------------------------------------------------
void WindowCalc(int Win_Array[], uint8 SE_data);
...
#if (NUM_FFT >= 256)
uint16 index, ADC_Index;
#endif
#if (NUM_FFT < 256)
uint8 index, ADC_Index;
#endif
uint16 BinNum;
...
{
#if (NUM_FFT >= 512)
	uint16 sin_index, sinB_index, g_cnt, s_cnt; // Keeps track of the proper
	uint16 indexA, indexB; // index locations for each
#endif // calculation
#if (NUM_FFT <= 256)
	uint8 sin_index, sinB_index, g_cnt, s_cnt; // Keeps track of the proper
	uint8 indexA, indexB; // index locations for each
#endif // calculation
	uint16 group = 2, stage = NUM_FFT/4;
	long CosVal, SinVal;
